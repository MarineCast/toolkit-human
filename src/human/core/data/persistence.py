...
from human.core.artifacts import (
    ArtifactRef,
...
from .contracts import DatasetSpec, ProcessingMode, ValidationReport
...
class ArtifactStore:
    ...
    def write_table(
        ...
        *,
        ...
        if destination.exists() and not force:
            raise FileExistsError(f"Artifact already exists; pass --force: {destination}")
        ...
        if not report.valid:
            ...
            atomic_write_json(quarantine / "validation.json", report.__dict__, overwrite=True)
    ...
    def reference_existing(
        ...
        *,
        ...
        report.require_valid()
        checksum = checksum_path(destination)
        file_count = len(list(destination.rglob("*.parquet"))) if destination.is_dir() else 1
        return (
            ArtifactRef(
                kind=spec.layer.value,
                dataset_id=str(spec.dataset_id),
    ...
    def write_manifest(self, manifest: RunManifest, destination: Path, *, force: bool) -> Path:
        return manifest.write(destination, overwrite=force)

    def is_resumable(self, artifact: ArtifactRef, spec: DatasetSpec, config_hash: str) -> bool:
        return (
