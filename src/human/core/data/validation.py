    ...
    if spec.schema is not None:
        for field in spec.schema:
            if not field.nullable and field.name in table.column_names:
                ...
                if null_count:
                    errors.append(f"Non-nullable column {field.name} has {null_count} null values")
                    schema_valid = False
    key_unique = True
    if spec.primary_key:
...
def validate_path(path: Path, spec: DatasetSpec) -> ValidationReport:
    if not path.exists():
        return ValidationReport(False, str(spec.dataset_id), errors=(f"Missing artifact: {path}",))
        ...
        try:
            ...
            if spec.primary_key:
                ...
                if missing:
                    ...
                    keys = pa.concat_tables(key_tables, promote_options="permissive").to_pandas()
                    null_count = int(keys.isna().any(axis=1).sum())
                    if null_count:
                        errors.append(f"Primary key has {null_count} rows with null values")
                        key_unique = False
