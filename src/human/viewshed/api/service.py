...
from human.core.data import StageResult, ValidationReport
from human.core.data.persistence import checksum_path

from ..config import load_app_config
from ..finalize.final_artifacts import final_artifact_paths_from_raw
...
def _stage_invocations(stage: str, request: ViewshedRequest) -> tuple[StageInvocation, ...]:
    # Service-level ``force`` authorizes replacement of the run manifest. It
    # must not implicitly invalidate or redownload large upstream DEM/CHM
    # inputs. Individual pipeline stages expose their own explicit
    # ``--overwrite`` flags for callers that intentionally want to rebuild
    # stage data.
    if stage == "build-distance-weights":
        return tuple(StageInvocation(stage, source_type=source) for source in ("land", "water"))
    if stage == "build-vegetation-path-weights":
        # Land vegetation is the canonical dual-surface canopy ratio. The
        # explicit-ray vegetation stage is used only to materialize the neutral
        # water factor in the default workflow.
        return (StageInvocation(stage, source_type="water"),)
    if stage == "terrain-weight":
        # Land terrain is produced with its matched canopy run by the canonical
        # dual-surface stage; this invocation owns water-source terrain only.
        return (StageInvocation(stage, source_type="water"),)
    if stage in {"finalize-viewshed-lookups", "export-static-maps"}:
        # These are inexpensive derived products owned by the registered run.
        # Always refresh them from the active canonical factors so an existing
        # output from an older config cannot silently survive a resumed build.
        # This does not invalidate or redownload any upstream raster inputs.
        return (StageInvocation(stage, overwrite=True),)
    return (StageInvocation(stage),)


def _stage_signature(config_hash: str, stages: Iterable[str]) -> str:
    payload = json.dumps(
        {"config_hash": config_hash, "stages": list(stages)},
        sort_keys=True,
        separators=(",", ":"),
    )
...
def process(request: ViewshedRequest) -> StageResult:
    ...
    signature = _stage_signature(app.config_hash, request.stages)
    if request.resume:
        refs = _resume_refs(
            manifest_path,
            request=request,
    ...
    for stage in request.stages:
        for invocation in _stage_invocations(stage, request):
            run_stage(app, invocation)
    ...
    refs = (
        _artifact_refs(expected_paths, request=request, config_hash=app.config_hash)
        if not missing
        else ()
    )
    report = ValidationReport(
