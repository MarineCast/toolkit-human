...
def cleanup_vegetation_intermediates(cfg: VegetationPathConfig) -> list[Path]:
    ...
    candidates.extend(
        [
            land_support_dir / f"LAND_COVER_CLASS_WEIGHTS.csv",
            ...
            data_dir / f"LAND_COVER_CLASS_LOOKUP.csv",
            data_dir / f"LAND_COVER_CLASS_WEIGHTS.csv",
...
def build_water_neutral_vegetation_weights(
    ...
    *,
    ...
    validate_final_artifact(paths.ocean_vegetation_weights, FINAL_SCHEMAS["vegetation_weights"])
    return {"water_vegetation_weights": paths.ocean_vegetation_weights}
