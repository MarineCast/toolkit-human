...
from human.core.config.paths import resolve_config_path

try:
    import yaml
except ImportError as exc:  # pragma: no cover
    raise ImportError(
        "PyYAML is required to read viewshed configuration files. "
        "Install it with: pip install pyyaml"
    ) from exc

from ..finalize.final_artifacts import tmp_dir_for_stage
from .paths import (
    get_h3_settings,
    get_run_version,
...
VALID_SOURCE_TYPES = {"land", "water"}
LOOKUP_SCHEMA = ("source_h3", "target_h3", "distance_km", "source_type")
DISTANCE_OUTPUT_SCHEMA = ("source_h3", "target_h3", "distance_km", "weight_distance")
...
def load_yaml(path: str | Path) -> dict[str, Any]:
    ...
    if not isinstance(data, dict):
        raise ValueError(f"YAML config must parse to a mapping/dict: {path}")
