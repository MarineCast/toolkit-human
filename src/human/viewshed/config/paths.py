...
Compatibility notes
-------------------
Some helpers, such as `load_config`, `get_bbox`, and
`resolve_existing_or_relative`, are retained as stable aliases for older
notebooks or module callers. Prefer the more explicit canonical names in new
code.
"""
...
from human.core.config.data import load_data_config
from human.core.config.paths import project_root, resolve_config_path
...
def resolve_existing_or_relative(value: str | Path, base_dir: Path) -> Path:
    return resolve_existing_or_relative_path(value, base_dir)
