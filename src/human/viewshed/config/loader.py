"""Shared infrastructure and finalization utilities for OrcaCast viewsheds.
...
VALID_VIEWSHED_BACKENDS = {"gdal", "xrspatial", "xarray"}
...
def load_app_config(config_path: str | Path) -> AppConfig:
    ...
    cfg = AppConfig(
        ...
        paths=PathsConfig(
            ...
            map_dir=resolve_path(paths.get("map_dir", "notebooks/viewshed/maps"), config_dir),
    ...
    if not cfg.paths.water_polygon_path.exists():
        raise FileNotFoundError(f"Missing water polygon: {cfg.paths.water_polygon_path}")
    if not cfg.paths.land_polygon_path.exists():
        raise FileNotFoundError(f"Missing land polygon: {cfg.paths.land_polygon_path}")
