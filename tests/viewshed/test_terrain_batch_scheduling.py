...
from human.viewshed.config.loader import BatchContext
from human.viewshed.prepare.area import batching as area
from human.viewshed.prepare.area import validation as batch_validation
from human.viewshed.weights import canopy_visibility
from human.viewshed.weights.terrain import (
    batches,
    cleanup,
    gdal,
    runner,
...
def test_h3_parent_batches_pack_nearby_remainders_deterministically() -> None:
    ...
    base = load_app_config("config/modeling/effort/viewshed.yaml")
    app = replace(
        base,
...
def test_batch_lookup_loads_only_requested_sources(tmp_path: Path) -> None:
    app = load_app_config("config/modeling/effort/viewshed.yaml")
